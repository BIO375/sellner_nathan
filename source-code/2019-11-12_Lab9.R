...
ggplot(data = fowler)+
  geom_histogram(aes(FERTILIZER), binwidth = 13)
ggplot(data = fowler)+
  geom_histogram(aes(YIELD), binwidth = 15)
...
modelgrass <- lm(YIELD ~ FERTILIZER, data = fowler)

autoplot(modelgrass)

grass_plus <- augment(modelgrass)
ggplot(data = grass_plus)+
...
ggplot(data = fowler, aes(x = FERTILIZER, y = YIELD)) +
  ...
  geom_smooth(method = "lm", level=0.95) +
  theme_bw()+
  labs( x = "Fertilizer", y = "Grass Yield")
